...
private:
  ...
  //new functions
  void neighboringHelp(BSTNode<Key,E>*,int,double,double,double) const ;
...
public:
 ...
 //print neighboring city
  void neighboringCity(double x,double y,double dis)
  {
      if(root ==NULL)
        cout<<"The CityDataBase is empty.\n";
      else
...
template<typename Key,typename E>
void BST<Key,E>::
neighboringHelp(BSTNode<Key,E>* root,int level,double x,double y,double dis) const{
	if (root == NULL)	return ;
	neighboringHelp(root->left(),level+1,x,y,dis);
	neighboringHelp(root->right(),level+1,x,y,dis);
	E it = root -> element();
	double disX = x-it.getx();
	double disY = y-it.gety();
	if(disX*disX+disY*disY<=dis*dis)
		cout<<it.getname()<<" x:"<<it.getx()<<" y:"<<it.gety()<<"\n";
}
